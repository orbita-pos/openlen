...
// cuerpo de `POST /api/agent` (el pin por turno del binario), justamente para
...
// lo que el binario enseña como `Effort level: auto (currently high)`.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function GET() {
  ...
  // 🔴 LA ESCALERA LA DICE EL MODELO, no una constante. Eso sí es del binario:
  // `E8(modelId)` -> `{supportsEffort, supportsMax, supportsXHigh, capLevels,
  // defaultEffort}`, leído de la entrada del catálogo de ESE modelo.
  //
...
export async function PUT(req: Request) {
  ...
  // el binario al volver a auto —«Cleared effort from settings»—, y deja la
