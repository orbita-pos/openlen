...
import { sealRelease } from "./html-engine";
...
// LAS PRUEBAS DE LA CSP SE RETIRARON el 2026-08-26 con la política.
//
// Medían `connect-src` abierto a `https: wss:` (decisión de Jesús del 24/08,
// como vercel.app, netlify.app y github.io, que no restringen ninguno),
// `form-action` cerrado a OpenLen, y el kill-switch `OPENLEN_PAGE_NETWORK=0`
// que revertía sin recompilar el nativo.
//
// Ya no hay política. Lo que el sellador sigue haciendo —quitar `<base>`, poner
// `rel=noopener`— se mide en lib/publish/csp-seal-gate.test.ts, contra el
// fichero que de verdad se escribe en disco.
