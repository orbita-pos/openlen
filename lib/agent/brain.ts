...
export function createAgentBrain(options: AgentBrainOptions): AgentBrain {
  ...
  // `CLAUDE_CODE_EFFORT_LEVEL` en el binario (ver `esfuerzo-efectivo.ts`).
  const env = options.env ?? process.env;
  // 🔴 Y SE RECORTA A LO QUE ESTE MODELO OFRECE, que es el último paso del
  // binario al resolver el nivel (`_7e(…, capLevels, supportsMax,
