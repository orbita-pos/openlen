...
/** LOS NIVELES, ordenados. Es el `Tm` de Claude Code, literal:
 ...
 *  `fE()` de Claude Code, y lo usan por igual el cable (para saber qué mandar) y
 ...
 *
 * El recorte es de Claude Code: `qf = Math.max(1024, Math.min(aD - 1, qf))`, con
...
/** La reserva de Claude Code, literal: `capLevels: ["low","medium","high"]`. */
...
export interface CapacidadDeEsfuerzo {
  ...
  /** A qué resuelve `auto` en este modelo. Es el `default_effort` de Claude Code,
...
/** Qué dial tiene ESTE modelo. El `E8(modelId)` de Claude Code. */
 ...
 * Code hace exactamente este recorte (`_7e(…, capLevels, supportsMax,
