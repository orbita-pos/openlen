...
const EDITAR_PAGINA_CONDUCTAS_SIN_RUNTIME = 'SIEMPRE QUE CAMBIES EL COMPORTAMIENTO de la página cableando una CONDUCTA (data-ol-calc y las demás), MANDA TAMBIÉN `prueba`: una lista corta (máx 6 pasos) de lo que tu código DEBE hacer, que se ejecuta en un navegador de verdad justo después de guardar. Cada paso: {clic:"#selector", veces?:N, escribe?:{"#campo":"valor"}, entonces:[{donde:"#selector", que:"cambia"|"contiene"|"es"|"visible"|"oculto", valor?:"texto"}]}. Ejemplo para una ruleta: [{clic:"#girar", entonces:[{donde:"#resultado", que:"cambia"}]}]. Para un carrito: [{clic:"#add", veces:3, entonces:[{donde:"#total", que:"es", valor:"3"}]}]. NO es opcional: se ejecuta de verdad y es la ÚNICA forma de saber si lo que cableaste FUNCIONA. Recoger errores sólo ve lo que EXPLOTA, y los dos fallos que de verdad pasan no explotan — una conducta mal cableada nace MUDA (el botón no hace nada, consola limpia) y una ruleta puede girar y no parar nunca.';
...
export function buildFunctionDeclarations(
  ...
  return [
    ...
    {
      name: "editar_pagina",
      description: documentOpsEnabled()
...
- LO QUE DE VERDAD NO SE PUEDE, que es poco y muy concreto: COBRAR (no hay pasarela de pagos: un carrito puede sumar, guardar y enseñar el total, pero el pago se cierra fuera — por WhatsApp, por transferencia o por un enlace de cobro que el dueño te dé); GUARDAR ENTRE VISITANTES (lo que tu script guarde vive en el navegador de QUIEN lo hizo: un carrito, un favorito o una puntuación son suyos y sólo suyos, no se comparten ni le llegan al dueño); y MANDAR CORREOS por tu cuenta (eso lo hace el formulario, que sí funciona). Un BUSCADOR dentro de la página SÍ se puede —es filtrar el DOM— y un blog también, si los artículos son secciones o páginas del sitio; lo que no hay es una base de datos de artículos, y para eso está Colecciones. Reservas, Pedidos, Comentarios, Cuentas y Broadcast SE RETIRARON: si te piden agendar, iniciar sesión o recibir pedidos, dilo y ofrece el WhatsApp o el formulario; JAMÁS digas que activaste uno de ellos. Y cuando construyas algo cuyo estado sólo vive en el navegador, DILO en la misma frase en que lo entregas — no después, y no en letra pequeña.
...
${PUBLISH_CONTRACT}`;
  ...
  return swapJsClauses(prompt, ["agente", "contrato-completo", "conductas"], effectiveEnv);
}
