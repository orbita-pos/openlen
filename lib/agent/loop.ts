 ...
 *
 * VISTO en Claude Code, que es la vara:
 *
 *   1. **Su bucle principal no tiene tope de pasos.** `maxTurns` es un campo
 *      OPCIONAL de la definición de un agente («…»). No hay defecto: la sesión corre
 *      hasta que el trabajo termina.
 *   2. **Lo que la acota es el CONTEXTO, y compactando CONTINÚA** en vez de
 *      parar: «…».
 *   3. **El dinero se topa por MES y por cuenta**, nunca por turno: «…», con
 *      auto-recarga.
 *   4. Y cuando un presupuesto sí se agota, no se tira nada: «…»
 ...
 * 3 (`…`), así que el tope por turno era un SEGUNDO muro
 ...
 * cliente, `…` dice que se puede continuar, y desde I6 el
