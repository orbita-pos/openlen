...
export interface AgentLoopArgs {
   ...
   *
   * Copiado del mecanismo de Claude Code (v2.1.260): el turno NO
 ...
 *
 * LA VARA, verbatim de Claude Code 2.1.260: «Your previous response
...
export async function runAgentLoop(args: AgentLoopArgs): Promise<AgentLoopResult> {
  ...
  const cerrarTurno = async (): Promise<AgentLoopResult | null> => {
    ...
    // evaluador, igual que Claude Code devuelve `not_met <razón>`— y el bucle
  ...
  while (true) {
    ...
    // `<new-diagnostics>` es un mensaje aparte, nunca el `tool_result`).
    //
