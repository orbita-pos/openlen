...
export function buildAgentContext(args: {
  ...
  if (args.attachedImage) {
    ...
    const seeLine = args.attachedImage.visible
      ...
      : "";
    imageBlock = `IMAGEN ADJUNTA DEL USUARIO: ${args.attachedImage.url}${altLine}${seeLine}\nEsta es una URL de imagen REAL que el usuario adjuntó explícitamente — colócala con editar_pagina usando esta URL EXACTA (verbatim) como src de un <img> (o como CSS background-image). NUNCA inventes ni cambies la URL. Y NO LA JUZGUES: la escribió el subidor de OpenLen, no el usuario. En desarrollo apunta al servidor del propio dueño (localhost) porque ahí no hay almacenamiento en la nube, y al publicar se hornea igual que cualquier otra — el publicador reconoce nuestras subidas por su ruta, no por su host. Que empiece por localhost NO es un motivo para negarse, ni para pedirle que la vuelva a subir «de otra forma»: es el mismo subidor y daría la misma dirección. Colócala. Si la página ya tiene un placeholder para esta imagen (un <div> con gradiente, una caja vacía con borde), REEMPLAZA ese elemento completo por el <img> — no lo anides adentro. Incluye siempre texto alt (usa el del usuario si lo dio; si no, infiérelo del contexto).\n\n`;
  }
