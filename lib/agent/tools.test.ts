...
describe("proponer_objetivo", () => {
  ...
  // no del binario, y el binario contesta esto de plano. En `ProposeGoal.call` la
  // ÚNICA guarda sobre un objetivo existente es `pendingGoalProposal` —una
  // propuesta SIN DECIDIR—; de `activeGoal` no hay ninguna comprobación. Y al
  // aprobar, su `Uve` supersede el anterior a propósito: «if (d !== undefined)
  // WF(d, "superseded")». Su descripción lo dice en una línea: «One goal is
  // active at a time; a newly approved or directly set proposal replaces the
  // current one».
  //
  ...
  // 🔴 LO QUE SÍ BLOQUEA, y es el eje del binario: una propuesta que el dueño
