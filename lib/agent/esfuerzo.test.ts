...
describe("la postura se traduce a un número, y `auto` resuelve a un nivel", () => {
  ...
  // fiel a una lectura EQUIVOCADA del binario: se tomó el `{type:"adaptive"}`
  // del eje de PRESUPUESTO por el `auto` del eje de NIVEL. Lo decide `c9t()`, y
  // su entrada es el modelo, no la elección de la persona. En el eje del nivel
  // el binario resuelve (`default_effort ?? "high"`) y manda.
  ...
  it("el número se recorta contra el techo de salida (regla del binario)", () => {
  ...
  // `NIVELES` es el `Tm` del binario y `auto` se añade aparte, no es un peldaño.
  ...
  it("el defecto deja niveles por encima (3 de 5, como el binario)", () => {
...
//
// El binario no ofrece los cinco a todo el mundo: `E8(modelId)` devuelve
...
describe("la capacidad de esfuerzo la dice el modelo", () => {
  ...
  // 🔴 LA RESERVA DEL BINARIO, LITERAL. Es la mitad que importa: sin ella un
