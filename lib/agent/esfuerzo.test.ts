...
describe("la postura se traduce a un número, y `auto` resuelve a un nivel", () => {
  ...
  // Claude Code resuelve (`default_effort ?? "high"`) y manda.
  ...
  // `NIVELES` es el `Tm` de Claude Code y `auto` se añade aparte, no es un peldaño.
...
//
// De Claude Code se copia DÓNDE VIVE LA DECISIÓN: `E8(modelId)` saca la escalera de
