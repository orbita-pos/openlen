...
import { CREATIVE_FONT_MOODS, CREATIVE_RADIUS_SCALES, CREATIVE_SPACING_SCALES, CREATIVE_TOKEN_ALLOWLIST } from "@/lib/generation/creative-registry";

const MAX_CREATIVE_LIST_LENGTH = 12;
const ColorSchema = z.string().regex(/^#[0-9A-Fa-f]{6}$/);
const HookIdSchema = z.string().regex(/^[a-z0-9]+(?:[-_:][a-z0-9]+)*$/).max(96);
const SafeTextSchema = z.string().min(1).max(180).refine(
  (value) => !/(<\/?[a-z][^>]*>|\b(?:https?:\/\/|www\.)|\b(?:javascript|data):)/i.test(value),
  "must not contain HTML, scripts, or URLs",
);
...
export const SkeletonInventorySchema = z.object({
  ...
  availableTokens: z.array(z.string().refine((token) => CREATIVE_TOKEN_ALLOWLIST.has(token), "must be an approved creative token")).max(MAX_CREATIVE_LIST_LENGTH),
  styleHooks: z.array(z.object({ id: HookIdSchema, selector: z.string().min(1).max(240), allowedProperties: z.array(z.string().min(1).max(64)).max(MAX_CREATIVE_LIST_LENGTH) }).strict()).max(MAX_CREATIVE_LIST_LENGTH),
...
const CssOverrideSchema = z.object({
  hookId: HookIdSchema,
  declarations: z.record(z.string().min(1).max(180).refine((value) => !/(?:<\/?[a-z][^>]*>|\b(?:https?:\/\/|www\.)|\b(?:javascript|data):|[{};])/i.test(value), "must not contain HTML, URLs, scripts, or free-form CSS")),
}).strict();
...
export const SkeletonAdaptationPlanSchema = z.object({
  ...
  tokens: z.record(z.string().refine((token) => CREATIVE_TOKEN_ALLOWLIST.has(token), "must be an approved creative token"), z.string().min(1).max(180).refine((value) => !/(?:<\/?[a-z][^>]*>|\b(?:https?:\/\/|www\.)|\b(?:javascript|data):|[{};])/i.test(value), "must not contain HTML, URLs, scripts, or free-form CSS")),
