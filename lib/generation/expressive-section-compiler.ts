...
function renderCopy(node: Extract<ExpressiveNode, { kind: "copy" }>, index: number, copy: Readonly<Record<string, string>>, motion: string): string {
  const classes = `olx-copy olx-${node.variant} olx-node-${index} ${SIZE_CLASS[node.size]} ${COLOR_CLASS[node.color]} olx-text-${node.align}${motion}`;
...
function renderNode(
  ...
  return `<div class="olx-decoration olx-node-${index} olx-shape-${node.shape} ${COLOR_CLASS[node.color]} ${SIZE_CLASS[node.size]} ${TRANSFORM_CLASS[node.transform]} ${BLEND_CLASS[node.blend]} olx-opacity-${node.opacity}${motion}" aria-hidden="true"></div>`;
}
