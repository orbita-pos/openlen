...
export const WitnessRecordSchema = z.object({
  ...
  // Gate-specific fields. Present only on step="gates" records.
  gateId: GateIdSchema.optional(),
  gatePassed: z.boolean().optional(),
  gateViolationCount: z.number().int().nonnegative().optional(),
  // Refine-specific fields. Present only on step="refine" records.
  refineAttempt: z.number().int().nonnegative().optional(),
  refinedBlockIndices: z.array(z.number().int().nonnegative()).optional(),
  remainingCriticalViolations: z.number().int().nonnegative().optional(),
});
...
export const LandingPageMetaSchema = z.object({
  ...
  // Quality-gate verdict. Absent on pre-Session-5 generations and on pages
  // generated by /api/regenerate-section (which re-uses the original meta).
  qualityGrade: QualityGradeSchema.optional(),
  gateResults: GatesAggregateResultSchema.optional(),
  /** Number of refine passes the orchestrator ran (0-2). */
  refineAttempts: z.number().int().nonnegative().optional(),
});
...
export const ProgressEventSchema = z.object({
  ...
  blockIndex: z.number().int().nonnegative().optional(),
  blockId: BlockIdSchema.optional(),
  // For "gates" step events. `gateId` identifies which of the six gates is
  // starting/finishing; `gatePassed` + `gateViolationCount` ride along on the
  // "completed" event for that gate so the client can update the UI without
  // waiting for the aggregate result.
  gateId: GateIdSchema.optional(),
  gatePassed: z.boolean().optional(),
  gateViolationCount: z.number().int().nonnegative().optional(),
  // For "refine" step events: which attempt this refers to.
  refineAttempt: z.number().int().nonnegative().optional(),
});
...
export type StepResultPayload =
  ...
  | { step: "gates"; data: z.infer<typeof GatesAggregateResultSchema> };
