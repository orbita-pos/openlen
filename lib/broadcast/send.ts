...
// Single-flight: each send waits for the previous to settle, so concurrent
// campaigns from different projects serialize rather than pile up.
let chain: Promise<unknown> = Promise.resolve();
function withSendSlot<T>(fn: () => Promise<T>): Promise<T> {
