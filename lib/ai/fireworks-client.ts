...
export function decodeFireworksUsage(value: unknown): ModelTokenUsage | undefined {
...
function jsonCandidates(content: string): unknown[] {
  try {
    ...
    // Compatible providers can wrap an otherwise-valid strict JSON value.
  }
  ...
  for (let index = 0; index < content.length; index += 1) {
    ...
    if (character === "}" || character === "]") {
      ...
      stack.pop();
      if (stack.length === 0) {
        try { candidates.push(JSON.parse(content.slice(start, index + 1))); } catch { /* retain no provider bytes */ }
        start = -1;
      }
    }
  }
    ...
    async request<T>(request: FireworksJsonRequest<T>): Promise<FireworksJsonResult<T>> {
      ...
      for (const attempt of ([1, 2] as const).slice(0, requestMaxAttempts)) {
        ...
        try {
          ...
          safeUsage = decodeFireworksUsage(decodedEnvelope);
