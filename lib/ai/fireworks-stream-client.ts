...
export interface FireworksStreamRequest {
   ...
   *
   *  Es la forma que tiene el binario de Claude Code, que resuelve la misma
   *  pregunta: su campo `effort` es `w.union([w.enum(["low","medium","high",
   *  "xhigh","max"]), w.number().int().min(1).max(1000)])` — un NOMBRE o un
  ...
  return {
    ...
    async *stream(request, opts = {}) {
      ...
      if (!response.ok || !response.body) {
        ...
        // 🔴 LA DEGRADACIÓN SILENCIOSA, que es lo que hace el binario. Si el
