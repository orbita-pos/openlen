...
.catch(function(s){d.remove();if(s===403){comprobarEstado();return}bubble("bot",T.error)})
...
fetch(C.api+"/api/assistant/"+C.sub,{headers:{accept:"application/json"}})
.then(function(r){return r.ok?r.json():Promise.reject(r.status)})
.then(function(j){
if(!j)return;
if(j.asistente===false){retirar(j.chat===true);return}
// El asistente sigue, el chat no: lo unico muerto es su boton, que hoy da 404.
if(C.handoff&&j.chat===false&&talk){try{talk.remove()}catch(e){}}})
.catch(function(){})}
comprobarEstado();
}catch(e){}})();</script>`;
}
