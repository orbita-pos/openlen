 ...
 *  the page already carries the profile contact widget (dedup), or this FAB is
...
export function bakeWhatsAppButton(html: string, cfg: WhatsAppButtonConfig): string {
  ...
  // The profile contact widget already owns the bottom corner — never stack two.
  if (html.includes("data-ol-contact-widget")) return html;
