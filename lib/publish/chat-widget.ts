...
const STRINGS: Record<string, ChatStrings> = {
  ...
  ko: { title: "메시지", login: "로그인", register: "가입", username: "사용자 이름", password: "비밀번호", displayName: "표시 이름 (선택)", loginBtn: "로그인", registerBtn: "계정 만들기", inviteOnly: "이 공간은 초대 전용입니다.", errBadUser: "잘못된 사용자 이름입니다.", errBadPass: "비밀번호가 너무 짧습니다.", errTaken: "이미 사용 중인 사용자 이름입니다.", errInvalid: "사용자 이름 또는 비밀번호가 틀렸습니다.", errRate: "시도가 너무 많습니다. 잠시 기다려 주세요.", search: "@사용자 검색", noConvos: "아직 대화가 없습니다.", noUsers: "사용자를 찾을 수 없습니다.", compose: "메시지 입력…", send: "보내기", back: "뒤로", logout: "로그아웃", open: "채팅 열기", close: "닫기", error: "문제가 발생했습니다.", messageBusiness: "비즈니스에 메시지", online: "온라인", typing: "입력 중…" },
  zh: { title: "消息", login: "登录", register: "注册", username: "用户名", password: "密码", displayName: "显示名称（可选）", loginBtn: "登录", registerBtn: "创建账户", inviteOnly: "此空间仅限邀请。", errBadUser: "用户名无效。", errBadPass: "密码太短。", errTaken: "该用户名已被占用。", errInvalid: "用户名或密码错误。", errRate: "尝试次数过多，请稍候。", search: "搜索 @用户名", noConvos: "还没有会话。", noUsers: "未找到用户。", compose: "写条消息…", send: "发送", back: "返回", logout: "退出", open: "打开聊天", close: "关闭", error: "出了点问题。", messageBusiness: "联系商家", online: "在线", typing: "正在输入…" },
  nl: { title: "Berichten", login: "Inloggen", register: "Registreren", username: "Gebruikersnaam", password: "Wachtwoord", displayName: "Weergavenaam (optioneel)", loginBtn: "Inloggen", registerBtn: "Account aanmaken", inviteOnly: "Deze ruimte is alleen op uitnodiging.", errBadUser: "Ongeldige gebruikersnaam.", errBadPass: "Het wachtwoord is te kort.", errTaken: "Die gebruikersnaam is al in gebruik.", errInvalid: "Gebruikersnaam of wachtwoord onjuist.", errRate: "Te veel pogingen. Wacht even.", search: "Zoek @gebruiker", noConvos: "Nog geen gesprekken.", noUsers: "Geen gebruikers gevonden.", compose: "Schrijf een bericht…", send: "Versturen", back: "Terug", logout: "Uitloggen", open: "Chat openen", close: "Sluiten", error: "Er ging iets mis.", messageBusiness: "Bericht naar het bedrijf", online: "Online", typing: "aan het typen…" },
};
...
+'.panel{background:#fff;border-radius:16px;border:1px solid #ececef;box-shadow:0 12px 48px rgba(0,0,0,.18);flex-direction:column;overflow:hidden;color:#1a1a1a}'
...
+'.hd{background:'+ACC+';color:#fff;padding:11px 14px;font-weight:600;display:flex;justify-content:space-between;align-items:center;gap:8px}'
+'.hd .tt{font-size:15px}'
...
+'.hd .x{font-size:22px;line-height:1;padding:0 6px}'
+'.bd{flex:1;display:flex;flex-direction:column;min-height:0;background:#fff}'
+'.auth{padding:18px;display:flex;flex-direction:column;gap:10px;overflow-y:auto}'
+'.tabs{display:flex;gap:6px}'
+'.tabs button{flex:1;min-height:36px;border:1px solid #d8d8dc;border-radius:9px;background:#fff;font-weight:600;font-size:13px;cursor:pointer;color:#1a1a1a}'
+'.tabs button.on{background:'+ACC+';color:#fff;border-color:'+ACC+'}'
+'.auth input{min-height:42px;border:1px solid #d8d8dc;border-radius:10px;padding:0 12px;font-size:14px}'
+'.auth input:focus{outline:2px solid '+ACC+';border-color:'+ACC+'}'
+'.auth .go{min-height:44px;border:0;border-radius:10px;background:'+ACC+';color:#fff;font-weight:600;cursor:pointer}'
...
+'.srch{padding:10px;border-bottom:1px solid #ececef;display:flex;flex-direction:column;gap:6px}'
+'.srch input{min-height:40px;border:1px solid #d8d8dc;border-radius:10px;padding:0 12px;font-size:14px}'
+'.srch input:focus{outline:2px solid '+ACC+';border-color:'+ACC+'}'
+'.res{max-height:170px;overflow-y:auto;border:1px solid #ececef;border-radius:10px}'
+'.convs{flex:1;overflow-y:auto}'
+'.conv{padding:11px 14px;border-bottom:1px solid #f0f0f2;cursor:pointer;display:flex;flex-direction:column;gap:1px}'
+'.conv:hover{background:#f7f7f8}'
+'.conv .cn{font-weight:600;font-size:14px}'
+'.conv .cu{color:#9a9aa0;font-size:12px}'
+'.empty{color:#9a9aa0;font-size:13.5px;padding:18px;text-align:center;display:flex;flex-direction:column;gap:10px;align-items:stretch}'
...
+'.tbar{padding:8px 12px;border-bottom:1px solid #ececef;display:flex;align-items:center;gap:8px}'
...
+'.tbar .who{font-weight:600;font-size:14px}'
+'.tbar .st{font-size:11px;color:#9a9aa0;min-height:13px}'
+'.tbar .st.on{color:#16a34a}'
+'.msgs{flex:1;overflow-y:auto;padding:12px;background:#f7f7f8;display:flex;flex-direction:column;gap:6px}'
+'.row{display:flex}'
+'.row.mine{justify-content:flex-end}'
+'.bub{max-width:78%;padding:8px 11px;border-radius:14px;font-size:14px;line-height:1.4;white-space:pre-wrap;word-wrap:break-word}'
+'.row.b .bub{background:#fff;border:1px solid #e8e8ea;border-bottom-left-radius:4px}'
+'.row.mine .bub{background:'+ACC+';color:#fff;border-bottom-right-radius:4px}'
+'.comp{display:flex;gap:8px;padding:10px;border-top:1px solid #ececef;background:#fff}'
+'.comp input{flex:1;min-height:42px;border:1px solid #d8d8dc;border-radius:10px;padding:0 12px;font-size:14px}'
+'.comp input:focus{outline:2px solid '+ACC+';border-color:'+ACC+'}'
+'.comp button{min-width:44px;min-height:42px;border:0;border-radius:10px;background:'+ACC+';color:#fff;font-weight:600;cursor:pointer}'
...
var hd=el("div","hd");hd.appendChild(el("span","tt",T.title));
var ha=el("div","ha");
...
jpost("/conversations/owner",{}).then(function(x){if(x.s===200&&x.j&&x.j.conversation){threadView(x.j.conversation.id,C.title||T.messageBusiness)}}).catch(function(){})}

function threadView(convId,who){
clearBody();loBtn.style.display="";
var selfId=me&&me.id;
var w=el("div","thread");
...
function add(m){if(!m||seen[m.id])return;seen[m.id]=1;var row=el("div","row "+(m.mine?"mine":"b"));row.appendChild(el("div","bub",m.body));msgs.appendChild(row)}
function pull(){
var u="/messages?conversationId="+encodeURIComponent(convId)+(lastCursor?"&since="+encodeURIComponent(lastCursor):"");
jreq(u).then(function(x){var list=(x.j&&x.j.messages)||[];list.forEach(add);if(x.j&&x.j.nextCursor)lastCursor=x.j.nextCursor;if(list.length)msgs.scrollTop=msgs.scrollHeight}).catch(function(){})}
...
es.addEventListener("message",onEvt);es.addEventListener("presence",onEvt);es.addEventListener("typing",onEvt);
