...
import { PickSchema, buildCatalog, pickSystemPrompt, type TemplateCatalogItem } from "./pick-template";
...
describe("PickSchema", () => {
  it("accepts a valid pick", () => {
    expect(PickSchema.safeParse({ templateId: "mirror", copy: COPY }).success).toBe(true);
  });
  ...
  it("accepts partial copy (lenient — fills omitted fields with null/[])", () => {
    const r = PickSchema.safeParse({ templateId: "mirror", copy: { business_name: "x", tagline_en: "t" } });
...
describe("buildCatalog / pickSystemPrompt", () => {
  ...
  it("embeds the catalog + the exact-id constraint in the prompt", () => {
    ...
    expect(prompt).toMatch(/MUST be one of the ids/);
  });
});
