...
describe("notifyBrokenSheet", () => {
  it("dispara UNA notificación con una dedupeKey estable por (proyecto, sheet)", async () => {
    ...
    await notifyBrokenSheet(
      { projectId: "p1", ownerUserId: "u1", sheetUrl: "https://s/1", reason: "timeout" },
      { schedule },
    );
    ...
    expect(event).toMatchObject({ type: "live_sheet_broken", projectId: "p1", recipientUserId: "u1" });
  });
  ...
  it("never-throw: si el schedule falla, no propaga (un aviso roto jamás rompe la corrida)", async () => {
    ...
    await expect(
      notifyBrokenSheet({ projectId: "p1", ownerUserId: "u1", sheetUrl: "x", reason: "y" }, { schedule }),
