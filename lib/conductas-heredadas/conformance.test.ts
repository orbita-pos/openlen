...
// lib/conductas-heredadas/build.ts (BEHAVIORS_SCRIPT_BUDGET_BYTES) — scripts/qa/
...
// lib/** salvo lib/conductas-heredadas/** — buscando `setAttribute("<attr>"`/
...
const COLLISION_EXCLUDE = ["lib/conductas-heredadas", "node_modules", ".next"];
 ...
 *  simples o dobles — o con `dataset.<camelCase>`, fuera de lib/conductas-heredadas/,
...
describe("colisión de namespace — runtimeAttrs no puede pertenecer a otro subsistema", () => {
  ...
  it.each(withRuntimeAttrs.flatMap((b) => (b.runtimeAttrs ?? []).map((attr) => [b.name, attr] as const)))(
    "%s: %s no aparece escrito con (set|toggle)Attribute ni dataset.<camelCase> fuera de lib/conductas-heredadas/",
    (name, attr) => {
      const offenders = findSetAttributeCollisions(attr);
      expect(
        offenders,
        `"${attr}" (runtimeAttrs de "${name}") aparece escrito con setAttribute/toggleAttribute/dataset FUERA de lib/conductas-heredadas/ en: ${offenders.join(", ")} — ` +
