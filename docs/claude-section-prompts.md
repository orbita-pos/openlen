...
- Design tokens live in :root and EVERYTHING is painted with var(...) so the section re-themes cleanly after insert. Use the OpenLen contract vocabulary (docs/openlen-contract.md) — these names SUPERSEDE any token names in the per-type briefs below: --bg, --surface, --surface-2, --fg, --fg-muted, --fg-faint, --border, --border-strong, --accent, --accent-r (COMMA R,G,B triplet, e.g. 62,207,142, for rgba(var(--accent-r), <a>)), --accent-ink (text/icons ON the accent), --radius, --font-display, --font-body, --font-mono. Paint every color/border/radius/font from these — NO raw hex outside :root (neutral structural shadows / macOS traffic-light dots excepted).
