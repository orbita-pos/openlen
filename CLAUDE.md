...
| `app/api/generate/` | POST — free-form AI generation → a complete HTML document, saved as a new project. **DeepSeek (Fireworks) writes the page**, not Gemini: `usesDeepSeek` in `lib/ai/provider-switch.ts` is opt-OUT, so only `OPENLEN_GENERATE_PROVIDER=gemini` goes back. And it is **not one call** — write → retry/repair-pass → vision critic (Gemini Flash, informs only unless `OPENLEN_VISION_CRITIC_REGEN=1`) → **one call and one credit per declared subpage**. |
| `app/api/templates/ai-design/` | POST — conversational page editing; Mode A ops / Mode B full rewrite. Same writer as `generate` (DeepSeek by default; `OPENLEN_CHAT_PROVIDER=gemini` is its own way back). ⚠️ **No longer what the Chat tab calls by default** — see `/api/agent` below; this is the opt-out path. |
...
| `lib/agent/` | **Len, the agent** — `loop.ts` + `brain.ts` (DeepSeek) + `catalog.ts` (the tool declarations) + `verify.ts` (the eyes: renders the page and looks at it). Reached through `POST /api/agent`. |
...
**Who runs what**: DeepSeek writes pages, Qwen looks at attached images, Gemini is for pixels — and for the creation critic, the one place still pending on that policy.
