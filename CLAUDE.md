...
OpenLen is a landing-page builder. Users describe a page (or pick a template, or paste their own HTML), edit, and publish to `<sub>.<publish host>` — the host is `PUBLISH_BASE_HOST`, and **production has been `openlen.app` since 2026-08-23** (⚠️ **only `.app` serves pages since 2026-09-10** — `61cec120` narrowed the Caddy wildcard to `*.openlen.app` and `*.openlen.com` now just 308-redirects there; this line used to say the two served the same folders, and that staleness already cost one wrong code comment. `.app` is a separate browser origin, which is the point). Never hardcode either — `lib/publish/base-host.ts` is the single source. The repo runs as a Next.js app self-hosted on Hetzner — **never Vercel**. Brand was renamed from "Inari Pages" on 2026-05-16; `Inari Watch` is a separate sister product (monitoring, not pages), don't conflate.
...
| `app/api/generate/` | POST — free-form AI generation → a complete HTML document, saved as a new project. The writer is whatever `lib/generation/model-policy.ts` maps its operation to — **read the table, never a model name written here**. It is **one write call**, plus **one call and one credit per declared subpage**. ⚰️ Esta línea decía «write → retry/repair-pass → vision critic (informs only unless `OPENLEN_VISION_CRITIC_REGEN=1`)». Las tres piezas están muertas: el crítico con visión se retiró el 2026-09-04 (`446cd428`), `repair-pass.ts` **ya no existe** (se borró con la pasada; esta línea decía «no tiene importadores» y también se quedó atrás), y `OPENLEN_VISION_CRITIC_REGEN` no lo lee ningún `.ts` — nunca existió como palanca. 🔴 **Crear NO tiene bucle, y es la regla, no una tarea pendiente**: la reparación automática la retiró Jesús el 2026-09-04 —era barata y funcionaba— porque «corrige el USUARIO, no nosotros»; el porqué entero está en el comentario de `app/api/generate/route.ts`. Lo medido se le DICE al usuario (`emit("medida")`), que es la otra mitad de esa regla. No re-proponer un ciclo de reparación sin petición explícita. |
...
| `app/api/agent/` | POST — **the Agent (Len), and what the Chat tab actually talks to.** A tool-calling loop (`lib/agent/loop.ts`) that edits the page, reads and writes the business profile, searches photos, and VERIFIES its own work by rendering the page and looking at the screenshot. Kill switch: `OPENLEN_AGENT=0` refuses, and the client falls back to `ai-design` for the rest of the session. Reasoning posture (`lib/agent/esfuerzo.ts`, `esfuerzo-efectivo.ts`) accepts `auto\|low\|medium\|high\|xhigh`; the operator override `OPENLEN_AGENT_EFFORT` pins it above the turn's and the user's saved value, and an unrecognized value is ignored rather than breaking the turn. |
...
| `infra/` | Hetzner stack — **Caddy** is the web tier (`infra/caddy/Caddyfile`): apex reverse-proxies to Next on :3000, `*.openlen.app` serves `/var/www/openlen/<sub>/` off disk and `*.openlen.com` only 308-redirects to it (since 2026-09-10). `len.openlen.com` is OUR site, its own block, off `/var/www/len-site/`. In the page-serving wildcard, only the paths with an explicit `handle` reach Next — everything else hits `try_files` and is answered with the STATIC page (measured 2026-09-17: `<sub>.openlen.app/api/assistant/x` → 200 text/html). `infra/nginx/` is the PRE-cutover legacy, kept for reference — do not edit it expecting an effect. Also systemd units + timers (`infra/app/`), DB box (`infra/db/`), DNS, runbooks. |
...
**Who runs what — `lib/generation/model-policy.ts` IS the answer, and it is a table.** **Three** roles (`reasoner`, `visualCritic`, `agent`) — `designer` was retired on 2026-09-06 along with the four operations that had zero callers (`creative_direction`, `page_planning`, `initial_section_program`, `visual_repair`), leftovers of two deleted pipelines; a guard (`model-policy-sin-huerfanas.test.ts`) now fails if an operation in the table has no caller. Every surface names an OPERATION and the table picks the role, the model and the reasoning effort. 🔴 **Never restate a model name in this file** — that is exactly how the previous version of this line went stale and then misled a session into "reasoning" from it instead of from the code. Read the table.
...
**The Canva-mode pivot was rolled back on 2026-05-24** — see [[canva-mode-decision]] memory for the full reasoning. Short version: the document-model engine (`lib/doc/*`) and the structured Canva inspector worked in principle but the conversion of polished HTML templates lost too much visual fidelity (animations, custom CSS, brand SVGs all stripped). User prefers the HTML visual quality. Everything related (`lib/doc/`, `components/workspace-v2/model/`, scripts/templates-convert-*, ai-edit endpoint, document-mode flag) was removed. The born-canonical normalizer (`lib/normalize.ts`, `normalizeBornCanonical`) is the ingestion path — `from-html` + `from-template` + `generate` all produce plain HTML projects. ⚠️ **Dónde corre, medido el 2026-09-05:** en `document-gate.ts:208`, y sólo cuando `policy.normalize !== false`. Las TRES superficies del modelo (Crear, Chat y Len) entran por `lib/page-engine/prepare.ts`, que pasa **`normalize: false`** — a lo que escribe el modelo NO se le normaliza. Lo llaman directo, además, las rutas de comunidad (`lib/community/seed.ts`, `store.ts`). ⚰️ Esta línea decía que **`lib/projects.ts` lo corre a la entrada**: lo importaba y no lo llamaba (import retirado el 2026-09-05). Downstream of it, `lib/page-engine/` is the shared pipeline (prepare → apply edits → persist) used by create, edit and the Agent alike. **Do not propose Canva-mode revival without explicit user request.**
